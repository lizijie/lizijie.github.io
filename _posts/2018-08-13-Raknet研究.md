...
在游戏战斗玩法中，会允许英雄移动的同时释放技能。我们使用可靠UDP发送角色位置和技能数据包。然而单纯的可靠UDP，会要求只有在前面的数据接收成功r后，才允许之后的数据继续发送，此时接收端出现较大的延迟。经过份析，如英雄位置与技能数据包在大多情况下，不存在很强的依赖关系。我们可以创建多个可靠UDP，即英雄位置与技能数据包分别在2个可靠UDP下传输。然而创建多个socket可能增加系统资源消耗，同时需要写额外的代码隐藏这层实现。
<br>
Raknet在单个连接上增加了传输通道的概念（复用socket），来解决上面问题，并且提高数据传输效率。不过传输通道仅在***完全按序系列模式(\*_ORDERED)***与***部分按序系列模式（\*_SEQUENCED）***下有作用，分别支持最大32个通道，即最大64个通道，并且它们之间互不影响。传输通道只面向对发送端，接收端看不到通道概念。(图片来源[^2])
