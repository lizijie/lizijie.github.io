...
- 默认主键有索引作用，ObjectId的生成算法，共12个字节，具备索引特性。
| 字节 | 含义 |
|-----|------|
| 4B  | 时间戳（秒级，文档创建时间） |
...
| 3B  | 自增计数器（同一秒内的序号） |
* **4 字节：时间戳（Timestamp）**
  * 表示自 Unix 纪元（1970年1月1日）以来的**秒数**。
  * **作用**：这使得 ObjectId 宏观上是**按时间递增**的。因为包含了时间信息，你甚至不需要额外的 `createdAt` 字段，可以直接通过客户端代码（如 `ObjectId.getTimestamp()`）提取文档的创建时间。
* **5 字节：随机值（Random Value）**
  * 在每个 MongoDB 客户端驱动（或数据库服务端）**进程启动时生成一次**的随机数。
  * **作用**：确保在不同机器、不同进程中生成的 ID 互不干扰。如果进程重启，该值会重新生成。
* **3 字节：递增计数器（Counter）**
  * 一个初始化为**随机值**的计数器。
  * **作用**：如果在**同一台机器、同一个进程、同一秒钟内**生成了多个 ObjectId，这个计数器就会递增。3 个字节最大可表示 $2^{24} = 16,777,216$ 个数字，足以支撑单进程单秒内极高并发的 ID 申请而不产生冲突。

早期的 ObjectId 算法（老版本/经典算法）
在 MongoDB 早期的版本中，12 字节分为 **4 个部分**：

* **4 字节：时间戳**（同上）
* **3 字节：机器标识码（Machine ID）**（通常基于主机名或网卡 MAC 地址的 Hash 值）
* **2 字节：进程 ID（PID）**
* **3 字节：递增计数器**

早期的算法在物理机时代运行良好，但在**容器化（如 Docker、Kubernetes）时代**暴露了缺陷：
在容器内，很多服务的进程 PID 往往固定为 `1`，且如果容器基于同一镜像动态伸缩，其生成的伪机器码（MAC 常常相似）也有可能重合。这就大幅降低了中间 5 个字节的“信息熵”（随机性），增加了 ID 碰撞的风险。因此，官方直接将这 5 个字节改为**进程启动时生成的纯随机数**，彻底杜绝了容器环境下的冲突问题。
