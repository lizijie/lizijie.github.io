...
-- bad
local M = {}
function M:settle_fight()
     local total = self:sum_teams(teams)
     ...
end

function M:sum_teams(teams)
     local result = 0
     for _, v in ipairs(teams) do
          result = result + v.score
     end
     return result
end
...
function M:settle_fight()
     ...
     local total = help.reduce(function(acc, x) return acc + x end, nums, 0)
     ...
end
```

- 对外的接口随时考虑【控制】与【逻辑数据】分层
```lua
-- bad
local M = {}
...
```
应该设计一个无任何限制条件即可进行游戏的接口，来满足上层多个触发点：正常业务逻辑/gm指令/快速测试/更其它玩法更复杂的业务控制
- 分离上层控制
- 底层接口单一原则
```lua
-- good
