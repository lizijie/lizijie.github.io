...
* [以密钥访问Git](#以密钥访问git)
* [用使用lefthook管理git hook](#用使用lefthook管理git-hook)
* [git常用大杂烩](#git常用大杂烩)
...
- `git config pull.ff only `Fast-Forward 当前(如下main分支)分支没有在你创建分支之后产生新提交，`pull`会自动`merge`
A — B — C        ← origin/main
         \
          D — E   ← local/main


- 否则。`pull`默认对不能Fast-Forward的提交
    - 合并提交`git config pull.rebase false`
    - 或者，变基提交`git config pull.rebase true`
          D — E     ← local/main
         /
...
sudo passwd git
# 创建仓库
sudo mkdir -p <存放仓库的目录>
sudo git init --bare <存放仓库的目录>/<仓库名称>.git
sudo chown -R git:git <存放仓库的目录>/<仓库名称>.git
# 将内容推送到无端仓库
cd my-project
git init
git add .
git commit -m "Initial commit"
git clone git@192.168.1.100:/srv/git/my-project.git
```
...
git push
```
- 为了保持父工程代码的稳定，期望子模块在主程固定是指定分地，而不是最新内容。子模块分配的目录记录了commitid，只要首次切好分支提交目录即可
```bash
git checkout [分支]
git add [子模块目录名]
git commit -m "xxx子模块xxx分支"
```
- 在父工程检查各个子模块的分支状态
```bash
...
- 删除子模块
``
# 从暂存区和工作区中删除子模块（这也会自动从 .gitmodules 中删除配置）
git rm -f path/to/submodule
# 物理删除子模块相关的 Git 缓存（防止后续重新添加时冲突）
...
* 将本地文件，还原到另一个分支的版本
```
git restore --source <另一个分支> -- path/to/file
```
* 删除暂存区文件
```
git rm -r --cache [文件路径]
