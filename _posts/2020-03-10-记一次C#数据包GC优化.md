...
**3. 带GC的API**
面对`.Net Framework`内部的GC，不要放弃优化，细细地看源代码（[.Net Framework线上源码](https://referencesource.microsoft.com/)），了解API的逻辑很可能找到优化的方法。比如有些接口同时提供了带GC和无GC的版本，如`Encoding.GetBytes(string)`， 见下图第一个重载带GC，第二个不带GC
